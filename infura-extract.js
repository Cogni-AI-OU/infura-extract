...
const baseCacheDir =
    process.env.MM_CACHE_DIR ||
    path.join(os.homedir(), '.cache', 'infura-extract')
const CACHE_DIR = path.join(baseCacheDir, network)

// Display cache location info
debug(
    `Using cache directory base: ${baseCacheDir}${
        process.env.MM_CACHE_DIR ? ' (from MM_CACHE_DIR)' : ' (default)'
    }`
)
...
async function getBlock(blockNumber) {
    ...
    const getShardedPath = (blockNum) => {
        ...
        if (!fs.existsSync(shardDir)) {
            try {
                ...
                debug(
                    `Error creating shard directory ${shardDir}: ${err.message}`
                )
    ...
    const { compressed: compressedCacheFile, legacy: legacyCacheFile } =
        getShardedPath(blockNumber)
    ...
    if (cacheDir.ready) {
        ...
        if (cacheDir.zstdAvailable && fs.existsSync(compressedCacheFile)) {
            try {
                debug(
                    `Reading compressed cached data from disk for block ${blockNumber}`
                )
        ...
        if (fs.existsSync(legacyCacheFile)) {
            try {
                debug(
                    `Reading legacy cached data from disk for block ${blockNumber}`
                )
    ...
    while (attempts < 3) {
        try {
            ...
            if (block && block.transactions) {
                debug(
                    `Successfully retrieved block ${blockNumber} with ${block.transactions.length} transactions`
                )
                ...
                if (cacheDir.ready) {
                    ...
                    const {
                        compressed: compressedCacheFile,
                    ...
                    if (cacheDir.zstdAvailable) {
                        try {
                            ...
                            debug(
                                `Saved and compressed block ${blockNumber} to disk cache`
                            )
                        } catch (writeErr) {
                            debug(
                                `Failed to write compressed cache: ${writeErr.message}`
                            )
                            // Fallback to uncompressed
                            try {
                                fs.writeFileSync(legacyCacheFile, jsonData)
                                debug(
                                    `Saved block ${blockNumber} to uncompressed disk cache`
                                )
                            } catch (fallbackErr) {
                                debug(
                                    `Failed to write cache: ${fallbackErr.message}`
                                )
                            }
                        }
                    } else {
                ...
                console.error(
                    `[ERROR] Retrieved empty or invalid block ${blockNumber} (attempt ${
                        attempts + 1
                    })`
                )
            ...
            console.error(
                `[ERROR] Error fetching block ${blockNumber} (attempt ${
                    attempts + 1
                }):`
            )
            console.error(
            ...
            if (
                ...
                console.error(
                    `Warning: The RPC endpoint for ${network} returned a non-JSON response.`
                )
                console.error(
                    "This often happens when the block doesn't exist or the network is experiencing issues."
                )
    ...
    console.error(
        `Failed to fetch valid block ${blockNumber} after 3 attempts.`
    )
    return null
}
...
async function main() {
    ...
    if (start > latestBlock) {
        console.warn(
            `[WARNING] Start block (${end}) exceeds the latest block (${latestBlock}). Adjusting start block to ${latestBlock}.`
        )
        start = latestBlock
    }
    ...
    if (end > latestBlock) {
        console.warn(
            `[WARNING] End block (${end}) exceeds the latest block (${latestBlock}). Adjusting end block to ${latestBlock}.`
        )
        end = latestBlock
    }
    ...
    debug(
        `Completed processing. Extracted ${addressCount} addresses from ${network}.`
    )
}
